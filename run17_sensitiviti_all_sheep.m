...
%% present niche: average
cond1_pr = (aggregate_pr >= 15 & aggregate_pr <= 2094);
cond1_tas = (aggregate_tas >= -4 & aggregate_tas <= 29);
cond1_sfcWind = (aggregate_sfcWind >= 1.7 & aggregate_sfcWind <= 6.0);
cond_hurs = (aggregate_hurs >= 37 & aggregate_hurs <= 69);
...
% 调整数据
    ...
    cond_pr = (data_pr >= 15 & data_pr <= 2094);
    cond_tas = (data_tas >= -4 & data_tas <= 30);
    cond_sfcWind = (data_sfcWind >= 1.7 & data_sfcWind <= 6.0);
    cond_hurs = (data_hurs >= 37 & data_hurs <= 66);
...
close all;
%save('./grazingniche/matdata/futsuggested by Max：average
...
percentage_world=turnover_decrease_value.rcp85/CNGarea_average_world
...
save('./grazingniche/matdata/outut_table_sheep.mat','output_table')
...
nexttile
  ...
  bar(y, 'FaceColor', 'flat', 'CData', colors);
  title("Africa")
  ylim([-3.5e6,1e6])
...
saveas(gcf, '/Users/lichaohui/Desktop/calculation/grazingniche/figuresnew/turnover_rcp85_allcontinents_sheep.svg');
%close all;
%% for 2 rcps (rcp26,85)
figure;
tiledlayout(2, 3, 'TileSpacing', 'compact', 'Padding', 'compact'); % 2x3 layout
...
for i = 1:length(continents)
    ...
    legend({'RCP2.6', 'RCP8.5'}, 'Location', 'northoutside', 'Orientation', 'horizontal'); % Legend for RCPs
    title([continent_title, ': Turnover under RCP Scenarios']);
    ylabel("Turnover");
...
for i = 1:length(continents)
    ...
    % Add labels and titles
    set(gca, 'XTickLabel', {'Increase', 'Decrease'}); % X-axis labels
    legend(rcp_labels, 'Location', 'northoutside', 'Orientation', 'horizontal'); % Legend for RCPs
    title([continent_title, ': Turnover under RCP Scenarios']);
    ylabel("Turnover");
    ylim(ylim_values(i, :)); % Set Y-axis limits for the current continent
end
