...
cond1_pr = (aggregate_pr >= 134 & aggregate_pr <= 2799);
cond1_tas = (aggregate_tas >= -1 & aggregate_tas <= 29);
cond1_sfcWind = (aggregate_sfcWind >= 1 & aggregate_sfcWind <= 5.6);
cond_hurs = (aggregate_hurs >= 42 & aggregate_hurs <= 67);
...
% 调整数据
    ...
    cond_pr = (data_pr >= 134 & data_pr <= 2799);
    cond_tas = (data_tas >= -1 & data_tas <= 29);
    cond_sfcWind = (data_sfcWind >= 1 & data_sfcWind <= 5.6);
    cond_hurs = (data_hurs >= 42 & data_hurs <= 67);
...
save('./grazingniche/matdata/outut_table_cattle.mat','output_table')
...
saveas(gcf, '/Users/lichaohui/Desktop/calculation/grazingniche/figuresnew/turnover_rcp85_allcontinents_cattle.svg');
%close all;
%% for 2 rcps (rcp26,85)
figure;
tiledlayout(2, 3, 'TileSpacing', 'compact', 'Padding', 'compact'); % 2x3 layout
