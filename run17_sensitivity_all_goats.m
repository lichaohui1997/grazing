...
cond1_pr = (aggregate_pr >= 11 & aggregate_pr <= 2136);
cond1_tas = (aggregate_tas >= -4 & aggregate_tas <= 30);
cond1_sfcWind = (aggregate_sfcWind >= 1.1 & aggregate_sfcWind <= 5.9);
cond_hurs = (aggregate_hurs >= 37 & aggregate_hurs <= 66);
...
% 调整数据
    ...
    cond_pr = (data_pr >= 11 & data_pr <= 2136);
    cond_tas = (data_tas >= -4 & data_tas <= 30);
    cond_sfcWind = (data_sfcWind >= 1.1 & data_sfcWind <= 5.9);
    cond_hurs = (data_hurs >= 36 & data_hurs <= 65);
...
save('./grazingniche/matdata/outut_table_goats.mat','output_table')
...
saveas(gcf, '/Users/lichaohui/Desktop/calculation/grazingniche/figuresnew/turnover_rcp85_allcontinents_goats.svg');
%close all;
%% for 2 rcps (rcp26,85)
figure;
tiledlayout(2, 3, 'TileSpacing', 'compact', 'Padding', 'compact'); % 2x3 layout
